#!/usr/bin/env python3
"""Credential-free cold preflight for the 400-call five-arm formal v2 manifest."""
...
def main() -> int:
    ...
    if len(requests) != 400:
        raise RuntimeError(f"expected 400 requests, built {len(requests)}")
    common_groups = Counter(
        (row["sequence_id"], row["event_index"], row["normalized_request_sha256"])
        for row in requests
    )
    if len(common_groups) != 80 or set(common_groups.values()) != {5}:
    ...
    status = {
        "schema": "cope-sequential-formal-cold-preflight-v2",
        "runtime_git_commit": runtime_commit,
        "manifest_sha256": hashlib.sha256(args.manifest.read_bytes()).hexdigest(),
        "sequence_units": 40,
        "arms": 5,
        "events_per_sequence": 2,
        "planned_provider_calls": 400,
