...
Every freeze validation reads the bound file bytes again. Stat-only hash caching is forbidden: the server filesystem was observed to preserve inode, size, mtime and ctime across rapid content changes. Rehashing large checkpoints adds I/O and CPU cost to each guarded boundary; a future optimization requires a separately verified immutability mechanism, not an assumed timestamp guarantee.
