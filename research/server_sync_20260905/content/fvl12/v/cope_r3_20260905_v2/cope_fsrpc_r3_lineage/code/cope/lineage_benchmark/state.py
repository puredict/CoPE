...
def _parse_patch(raw: Mapping[str, Any], event: LineageEvent, revision: int) -> List[Dict[str, Any]]:
    ...
    return [dict(item) for item in raw["ops"]]
...
def apply_patch_document(
    ...
    for op in ops:
        name = op.get("op")
        if name == "suspend":
            if set(op) != {"op", "slot_id"}:
                raise StateValidationError("suspend fields must be op,slot_id")
            sid = str(op["slot_id"])
            slot = slots.get(sid)
            if slot is None or slot.mode != "active":
            ...
            if set(op) != {"op", "slot_id"}:
                raise StateValidationError("restore fields must be op,slot_id")
            sid = str(op["slot_id"])
            slot = slots.get(sid)
            if slot is None or slot.mode != "suspended":
            ...
            if set(op) != {"op", "slot_id", "new_slot"}:
                raise StateValidationError("override fields must be op,slot_id,new_slot")
            sid = str(op["slot_id"])
            old = slots.get(sid)
            if old is None or old.mode not in {"active", "suspended"}:
                raise StateValidationError(f"cannot override {sid!r} from its current mode")
            new_raw = op["new_slot"]
            if not isinstance(new_raw, Mapping):
                raise StateValidationError("override.new_slot must be an object")
            allowed = {"slot_id", "logical_id", "grounding", "priority", "payload"}
            if set(new_raw) != allowed:
                raise StateValidationError(
                    f"override.new_slot fields must be {sorted(allowed)}"
                )
            ...
            if set(op) != {"op", "from_slot_id"}:
                raise StateValidationError("restore_root fields must be op,from_slot_id")
...
def active_plan(state: WorkflowState, critical_logical_id: str) -> List[Dict[str, str]]:
    active = [
        slot for slot in state.slots.values()
