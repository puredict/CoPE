...
class Synthetic2DBackend(SimulatorRepairBackend):
    ...
    @property
    def cfg(self):
        """SceneConfig of the leg currently being executed.

        `SharedRepairEngine` reads `env.cfg` to size its control limits. Before
        the first leg begins there is no leg env yet, so a default scene is
        returned; `begin_leg` then rebinds the engine via `begin_leg(env)`.
        Without this the synthetic backend could not be driven through
        `run_backend_episode`, which meant the server code path had no
        CPU-testable route at all.
        """
        if self.env is not None:
            return self.env.cfg
        from cope.benchmark.basket_world import leg_config
        return leg_config("milk", "basket_A")
