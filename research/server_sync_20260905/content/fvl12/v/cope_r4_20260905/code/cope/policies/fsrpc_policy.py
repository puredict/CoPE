...
class FSRPCPolicy:
    ...
    def regenerate(self, inp: AdaptationInput) -> RegeneratedState:
        ...
        # A WITHDRAWN temporary redirection (I5). FSR-PC must receive the same
        # semantics as CoPE: the detour target is dropped and the goal reverts
        # to the grounding it had before the temporary override. Withholding
        # this would make the baseline fail the condition for a reason that has
        # nothing to do with regeneration.
        withdrawn = {canon(g) for g in upd.get("cancel_redirect", [])}
        reverted: Dict[str, Optional[str]] = {}
        for gid in withdrawn:
            for e in reversed(inp.event_history or ()):
                red = (e.get("update", {}) or {}).get("redirect", {}) or {}
                for k in red:
                    if canon(k) == gid:
                        reverted[gid] = None      # revert to the ORIGINAL target
                        break
                if gid in reverted:
                    break
        ...
        for gid in sorted(set(live) | completed
            ...
            if gid in withdrawn:
                # revert to the pre-detour grounding recorded in the payload
                tgt = base_payload.get("original_target", target)
            ...
            if gid in redirect:
                # symmetric to CoPE: remember the pre-detour grounding so a
                # later withdrawal can revert to it
                payload["original_target"] = base_payload.get(
                    "original_target", base_payload.get("target"))
            elif gid in withdrawn:
                payload.pop("original_target", None)
            slots.append(ConstraintSlot(
                id=new_id, grounding=f"place_{obj}_in_{tgt}", mode=mode,
                priority=(old.priority if old else Priority.SOFT),
                source=SlotSource.INITIAL_TASK, payload=payload))
