...
LEGAL = {
    ...
    # `suspended -> overridden` is legal: a goal that has been set aside may be
    # covered by a temporary stand-in (condition I5 -- basket_B is unavailable,
    # so butter is temporarily redirected to basket_C). The original keeps its
    # identity and its restore path; the cover participates in its place. This
    # is a lifecycle-table entry, not a change to any operator: Override's side
    # effects are untouched, and EXPIRED remains terminal.
    SlotMode.SUSPENDED:  {SlotMode.SUSPENDED, SlotMode.ACTIVE,
                          SlotMode.OVERRIDDEN, SlotMode.EXPIRED},
