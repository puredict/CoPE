...
class ConstraintStateToRepairGoalCompiler:
    ...
    def compile(self, *, before: ConstraintState, after: ConstraintState,
        ...
        # `target_changes` is derived from the canonical LIVE target, so the
        # `target_relocated` predicate cannot depend on whether a method
        # expressed the change as an override or as a re-emitted slot.
        target_changes = self._canonical_target_changes(before, after)
        preds = self._predicates(ctx, world, transitions, target_changes)
        ...
        # --- representation-neutral target-change check ---------------------
        # CoPE expresses a withdrawn detour as `expire(cover) + restore(original)`
        # while FSR-PC re-emits one slot whose target reverts. Both are correct
        # readings of the same event, but they land on different mode branches
        # above, which would hand the two arms DIFFERENT repair goals for the
        # identical semantic change (found by
        # `test_paired_arms_compile_identical_repair_goals[I5]`).
        # Comparing the canonical live target directly removes that asymmetry.
        for cid, old_t, new_t in target_changes:
            if "aligned_to_current_target" in goal.required_true:
                continue
            goal = goal.union(RepairGoal(
                ...
                label="retarget"))
            sources.append(RequirementSource(
                slot_id=cid, transition="target_changed",
                requirement="aligned_to_current_target",
                reason=f"the live grounding of {cid} moved from {old_t} to "
                       f"{new_t}; the program must adopt the current target "
                       f"contract before resuming"))
    ...
    @staticmethod
    def _canonical_target_changes(before: ConstraintState,
                                  after: ConstraintState
                                  ) -> List[Tuple[str, Any, Any]]:
        """Canonical goals whose LIVE target changed, whatever the mode path.

        "Live" prefers ACTIVE, then SUSPENDED, then OVERRIDDEN, and ignores
        EXPIRED records, so a withdrawn cover does not mask the goal underneath.
        """
        order = {SlotMode.ACTIVE: 0, SlotMode.SUSPENDED: 1,
                 SlotMode.OVERRIDDEN: 2}

        def live(state: ConstraintState) -> Dict[str, Any]:
            out: Dict[str, Any] = {}
            for sid in state.ids():
                s = state.get(sid)
                if s.mode is SlotMode.EXPIRED or "object" not in s.payload:
                    continue
                c = canonical_id(sid)
                cur = out.get(c)
                if cur is None or order[s.mode] < order[cur.mode]:
        ...
        for cid, new in sorted(a.items()):
            old = b.get(cid)
            if old is None:
                continue
            ot, nt = old.payload.get("target"), new.payload.get("target")
            if ot != nt:
                changed.append((cid, ot, nt))
        return changed
    ...
    def _predicates(self, ctx, world: Dict[str, Any],
                    ...
                    target_changes: Optional[List[Tuple[str, Any, Any]]] = None
                    ) -> WorldPredicates:
        ...
        # representation-neutral: a canonical live target that changed IS a
        # relocation, however the method chose to express it
        relocated = relocated or bool(target_changes)
