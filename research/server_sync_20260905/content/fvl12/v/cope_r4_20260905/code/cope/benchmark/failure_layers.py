...
def classify(*, task_metrics: Dict[str, Any], schedule: Dict[str, Any],
    ...
    # ---------- physical layer -------------------------------------------
    def leg_reason(leg: Dict[str, Any]) -> str:
        pick = leg.get("pick", {}) or {}
        return str(leg.get("reason") or leg.get("failure_reason")
                   or pick.get("failure_reason") or "")

    def leg_grasp_acquired(leg: Dict[str, Any]) -> Any:
        if "grasp_acquired" in leg:
            return leg.get("grasp_acquired")
        return (leg.get("pick", {}) or {}).get("grasp_acquired")

    # The real LIBERO harness nests the oracle result under ``pick`` and uses
    # ``failure_reason``; the CPU harness stores flat ``reason`` fields.  Both
    # schemas must classify the same physical failure at the controller layer.
    grasp_failures = [l for l in legs
                      if leg_reason(l).startswith("grasp")
                      or leg_grasp_acquired(l) is False]
    ...
    if infrastructure_errors or not required_artifacts_present or undelivered:
        ...
        reasons = sorted({leg_reason(l) for l in grasp_failures
                          if leg_reason(l)})
        if tm.get("collision"):
