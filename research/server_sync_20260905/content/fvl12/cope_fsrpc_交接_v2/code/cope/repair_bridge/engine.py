...
class SharedRepairEngine:
    ...
    def __init__(self, env, trace, *, horizon_budget: int = 200,
        ...
        # Optional backend adapter. Synthetic2D keeps the frozen verifier;
        # a real simulator supplies an interface-compatible verifier that
        # performs checkpoint-isolated rollouts in that simulator.
    ...
    def repair(self, *, before: ConstraintState, after: ConstraintState,
        ...
        manager = RepairManager(
            ...
            generator=generator, cfg=self.cfg,
            horizon_budget=self.horizon_budget)
        if self.verifier_factory is not None:
            manager.verifier = self.verifier_factory(
                backend=self.env,
                params=params,
                horizon_budget=self.horizon_budget,
            )
