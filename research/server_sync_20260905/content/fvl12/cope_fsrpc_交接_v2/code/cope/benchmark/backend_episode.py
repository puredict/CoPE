...
def run_backend_episode(
    ...
    *,
    ...
    def pending_updates() -> List[Tuple[str, Dict[str, Any]]]:
        ...
        trigger = canonical_id(cond.trigger_after_goal or "")
        if "u1" not in fired and (not trigger or trigger in completed_goal_ids):
            pending.append(("u1", cond.update))
        trigger2 = canonical_id(cond.second_trigger_after_goal or "")
        if (
            cond.second_update is not None
            and "u2" not in fired
            and ("u1" in fired or bool(pending))
            and trigger2 in completed_goal_ids
        ):
            pending.append(("u2", cond.second_update))
        return pending
