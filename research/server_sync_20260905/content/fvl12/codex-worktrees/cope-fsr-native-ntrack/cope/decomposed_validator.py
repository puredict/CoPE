...
COMMITMENT_FIELDS = {
    "id", "predicate", "grounding", "lifecycle_status", "source", "owner",
    "authority", "valid_from", "valid_until", "dependencies", "support_links",
    "override_links", "supersession_links",
}
...
def decomposed_violations(
    ...
    for record_id, record in post_commitments.items():
        if set(record) != COMMITMENT_FIELDS:
            add("identity_lifecycle", f"commitment_fields_noncanonical:{record_id}")
