...
FIELDS = [
    ...
    "latency_seconds", "validator_calls", "repair_success", "proposal_bytes",
    "call_order_position", "parse_or_validation_error",
]
    ...
    if provider_status == "ok":
        try:
            if arm == "cope":
                ...
                compact = parse_proposal(invocation.parsed_output, case.pre_state, case.event)
                parser_valid = True
                compact_result = execute_compact_transaction(
                    compact, case.pre_state, case.event, validate_and_compile
                )
                candidate = compact_result.post_state
                directive = compact_result.directive
                validator_calls = int(compact_result.receipt["validator_calls"])
            else:
    ...
    return {
        ...
        **flags,
        ...
        "proposal_bytes": (
            ...
            if isinstance(invocation.parsed_output, dict)
            else 0
        ),
        "call_order_position": "",
...
def run_triplet(
    ...
    *,
    ...
    order = ["cope", "compact_tx", "fsr_pc"]
    offset = order_index % 3
    call_order = order[offset:] + order[:offset]
    invocations: dict[str, ProviderInvocation] = {}
    for arm in call_order:
        if arm == "cope":
            invocations[arm] = provider.patch(common, case.pre_state)
        elif arm == "compact_tx":
            invocations[arm] = provider.compact(common)
        else:
            invocations[arm] = provider.regenerate(common)
    rows = [evaluate(case, arm, invocations[arm], provider) for arm in order]
    expected_payload = common.as_payload()
    fairness = bool(
        len({row["input_hash"] for row in rows}) == 1
        and all(call.raw_request.get("recovery_input") == expected_payload for call in invocations.values())
        and len({row["common_input_bytes_sha256"] for row in rows}) == 1
        and len({row["normalized_request_sha256"] for row in rows}) == 1
        and all(call.retry_count == 0 for call in invocations.values())
    )
    for row in rows:
        row["fairness_pass"] = fairness
        row["call_order_position"] = call_order.index(row["arm"])
    return rows
...
def write_fairness(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    fields = ["case_id", "triplet_rows", "input_hash_equal", "common_bytes_equal", "normalized_request_equal", "fairness_pass"]
    with path.open("x", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
        for case_id, pair in by_case.items():
            writer.writerow({
                ...
                "fairness_pass": len(pair) == 3 and all(row["fairness_pass"] for row in pair),
            })
...
def write_preflight_fairness(
    ...
    with path.open("x", newline="", encoding="utf-8") as handle:
        ...
        for case in cases:
            ...
            compact = provider.build_audited_request("compact", common, provider.compact_contract)
            full = provider.build_audited_request("regenerate", common, provider.full_contract)
            passed = bool(
                patch["recovery_input"] == compact["recovery_input"] == full["recovery_input"] == common.as_payload()
                and len({patch["common_input_message_sha256"], compact["common_input_message_sha256"], full["common_input_message_sha256"]}) == 1
                and len({normalized_raw_request_hash(patch), normalized_raw_request_hash(compact), normalized_raw_request_hash(full)}) == 1
            )
            all_pass = all_pass and passed
            writer.writerow({
                "case_id": case.case_id, "triplet_rows": 3, "input_hash_equal": True,
                "common_bytes_equal": len({patch["common_input_message_sha256"], compact["common_input_message_sha256"], full["common_input_message_sha256"]}) == 1,
                "normalized_request_equal": len({normalized_raw_request_hash(patch), normalized_raw_request_hash(compact), normalized_raw_request_hash(full)}) == 1,
...
def write_blocked_results(
    ...
    with path.open("x", newline="", encoding="utf-8") as handle:
        ...
        for case_index, case in enumerate(cases):
            ...
            offset = case_index % 3
            call_order = order[offset:] + order[:offset]
            for arm, mode, contract in arm_specs:
                ...
                writer.writerow({
                    ...
                    "proposal_bytes": 0, "call_order_position": call_order.index(arm),
                    "parse_or_validation_error": "",
                })


def write_blocked_aggregate(path: Path, assigned: int) -> None:
    fields = ["arm", "assigned", "configuration_blocked", "method_evaluable", "provider_ok", "provider_outage_or_timeout", "first_pass_valid", "semantic_correct", "unauthorized_or_stale_edit", "progress_corruption", "continuity_error", "prompt_tokens", "completion_tokens", "proposal_bytes", "latency_seconds", "validator_calls"]
    with path.open("x", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=fields, lineterminator="\n")
        writer.writeheader()
...
def main() -> int:
    ...
    if not os.environ.get(provider.api_key_env):
        ...
        with (args.output_dir / "06_FAILURE_TAXONOMY.csv").open("x", newline="", encoding="utf-8") as handle:
            writer = csv.writer(handle, lineterminator="\n")
            writer.writerow(["arm", "family", "category", "count"])
            writer.writerow(["all", "configuration", "credential_unavailable", 3 * len(cases)])
        status = {
            "smoke_gate_pass": False, "expanded": False, "assigned_triplets": len(cases),
    ...
    for case in smoke:
        rows.extend(run_triplet(case, provider, order_index=case_order[case.case_id]))
    smoke_gate = bool(
        len(rows) == 3 * len(smoke)
