...
cfg = Cfg()
out = Path("/home/lijingsu/vla/smoke_outputs/disturbance")
...
sim = env.env.sim; model_mj = sim.model; data = sim.data
body_name = "tomato_sauce_1_main"; joint_name = "tomato_sauce_1_joint0"
body_id = model_mj.body_name2id(body_name)
rows = []
for t in range(12):
    event = None
    if t == 6:
        qpos = data.get_joint_qpos(joint_name).copy()
        qpos[0] += 0.10; qpos[1] += 0.05
        data.set_joint_qpos(joint_name, qpos); sim.forward()
        event = {"type": "target_displacement", "joint": joint_name, "delta_xy": [0.10, 0.05]}
    target_pos_before_action = data.body_xpos[body_id].copy().round(6).tolist()
    if t < 3:
    ...
    row = {
        ...
        "phase": phase,
        "event": event,
        "target_pos_before_action": target_pos_before_action,
        "reward": float(reward),
        "done": bool(done),
    ...
    if done: break
env.close()
result = {"task": task_description, "disturbance": "target_displacement at t=6", "rows": rows}
(out / "disturbance_smoke.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
print("wrote", out / "disturbance_smoke.json", flush=True)
